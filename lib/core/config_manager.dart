...
class ConfigManager {
  ...
  Map<String, dynamic>? _uiSettings;
  ...
  /// 获取 UI 设置
  Map<String, dynamic> get uiSettings =>
      _uiSettings == null ? {} : _deepCopyMap(_uiSettings!);

  /// 设置 UI 配置
  Future<void> setUISettings(Map<String, dynamic> settings) async {
    _uiSettings = _deepCopyMap(settings);
    await saveConfig();
  }
  ...
  Future<void> loadConfig() async {
    try {
      ...
      if (await file.exists()) {
        ...
        if (json['uiSettings'] is Map) {
          _uiSettings = _deepCopyMap(Map<String, dynamic>.from(json['uiSettings']));
        }
