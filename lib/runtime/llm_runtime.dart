...
extension LLMRuntimeStructuredStream on LLMRuntime {
  /// 结构化流式输出（delta/metrics/finish/error）
  Stream<LLMStreamEvent> completeStreamEvents(
    String prompt, {
    GenerationConfig? config,
  }) {
    return _wrapTextStreamAsEvents(
      streamFactory: () => completeStream(prompt, config: config),
      requestIdPrefix: 'complete',
    );
  }

  /// 结构化流式对话（delta/metrics/finish/error）
  Stream<LLMStreamEvent> chatStreamEvents(
    List<ChatMessage> messages, {
    GenerationConfig? config,
  }) {
    return _wrapTextStreamAsEvents(
      streamFactory: () => chatStream(messages, config: config),
      requestIdPrefix: 'chat',
    );
  }

  Stream<LLMStreamEvent> _wrapTextStreamAsEvents({
    required Stream<String> Function() streamFactory,
    required String requestIdPrefix,
  }) async* {
    final requestId = _LLMRequestId.next(requestIdPrefix);
    int sequence = 0;
    ...
    try {
      await for (final chunk in streamFactory()) {
        ...
        completionTokens += _estimateTokens(chunk);
