...
if [ -n "$ARCHIVE_PREFIX" ]; then
    if [ -z "$ARCHIVE_OUTPUT_DIR" ]; then
        echo "❌ 目录归档模式需要提供 ARCHIVE_OUTPUT_DIR"
        exit 1
    fi

    first_part="${ARCHIVE_PREFIX}.part000"
    checksum_path="${ARCHIVE_PREFIX}.sha256"

    if [ ! -f "$first_part" ]; then
        echo "❌ 未发现归档分片: $first_part"
        exit 1
    fi
    ...
    echo "----------------------------------------"
    echo "发现归档分片组，目标: $ARCHIVE_PREFIX"
    echo "正在合并..."

    cat "${ARCHIVE_PREFIX}.part"* > "$ARCHIVE_PREFIX"

    if [ -f "$checksum_path" ]; then
        echo "正在校验归档..."
        (
            cd "$(dirname "$ARCHIVE_PREFIX")"
            shasum -a 256 -c "$(basename "$checksum_path")"
        )
    fi

    echo "正在解压到: $ARCHIVE_OUTPUT_DIR"
    tar -xzf "$ARCHIVE_PREFIX" -C "$ARCHIVE_OUTPUT_DIR"
    rm -f "$ARCHIVE_PREFIX"

    if [ "$ARCHIVE_KEEP_PARTS" != "1" ]; then
        echo "正在清理分片文件..."
